...
import { sendOTP, verifyOTP, resetPassword } from '@/lib/api/services/member.service'
...
function StepIndicator({ current }: { current: 1 | 2 | 3 }) {
  const steps = ['กรอกอีเมล', 'ยืนยัน OTP', 'รหัสผ่านใหม่']
  return (
...
export default function ForgotPasswordPage() {
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (!newPassword || newPassword.length < 8) errs.newPassword = 'รหัสผ่านต้องมีอย่างน้อย 8 ตัวอักษร'
    ...
    >
      ...
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <p style={{ fontFamily: 'var(--font-thai)', fontSize: 14, color: '#7b7b7b', margin: 0 }}>
          {step === 1 && 'กรอกอีเมลที่ใช้ลงทะเบียน เราจะส่ง OTP ให้'}
          {step === 2 && `ส่ง OTP ไปที่ ${email}`}
      ...
      {step === 3 && (
        <form
          ...
          style={{ display: 'flex', flexDirection: 'column', gap: 16, width: '100%' }}
        >
          <Field label="รหัสผ่านใหม่" error={passwordErrors.newPassword}>
          ...
          <Field label="ยืนยันรหัสผ่านใหม่" error={passwordErrors.confirmPassword}>
            <div style={{ position: 'relative' }}>
              <FieldInput
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            <button
              type="button"
              onClick={() => { setStep(2); setPasswordErrors({}) }}
              style={{
                flex: 1,
                height: 48,
                borderRadius: 8,
