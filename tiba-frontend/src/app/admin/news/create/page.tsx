...
export default function AdminNewsCreatePage() {
  ...
  const createMutation = useMutation({
    mutationFn: (fd: FormData) => contentService.createNews(fd),
    onSuccess: () => {
    ...
    <div style={{ fontFamily: F, maxWidth: 760, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', fontFamily: F, margin: 0 }}>สร้างข่าวสารและบทความ</h1>
        </div>
      ...
      <div style={{ backgroundColor: '#fff', borderRadius: 12, border: '1px solid #e5e7eb', padding: '30px', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div
            ...
            style={{ width: '100%', height: 180, borderRadius: 10, backgroundColor: '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', overflow: 'hidden', border: '2px dashed #c4b5fd' }}
          >
            {preview
              ...
              : (
                <div style={{ textAlign: 'center', color: '#7c3aed' }}>
                  ...
                  <div style={{ fontSize: 16, fontFamily: F }}>คลิกเพื่ออัปโหลดรูปหน้าปก</div>
                  <div style={{ fontSize: 14, color: '#a78bfa', marginTop: 4, fontFamily: F }}>PNG, JPG, WEBP (แนะนำ 1200×630)</div>
                </div>
              )
            }
          </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <select
            value={articleType}
            onChange={e => setArticleType(e.target.value as 'news' | 'blog')}
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <textarea
            ...
            style={{ ...inp, height: 'auto', minHeight: 200, padding: '10px 12px', resize: 'vertical' }}
          />
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, paddingTop: 18, borderTop: '1px solid #f3f4f6' }}>
          ...
          {isPublished && <span style={{ fontSize: 14, color: '#16a34a', fontFamily: F }}>บทความนี้จะถูกเผยแพร่ทันที</span>}
        </div>
      </div>
    </div>
  )
